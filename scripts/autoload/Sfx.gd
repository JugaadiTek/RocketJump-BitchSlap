...
## Sfx (autoload)
##
## Every sound in the game is synthesised here at startup rather than loaded from
## audio files - the project ships no .wav/.ogg assets, and generating them keeps
## it that way while still giving each weapon a distinct voice.
##
...
func play_3d(sound: String, position: Vector3, pitch: float = 1.0, volume_db: float = 0.0,
	...
	var stream: AudioStream = _library.get(sound)
	if stream == null:
		return
...
func _build_library() -> void:
	...
	_library["jump"] = _sound(_sweep(0.16, 260.0, 460.0, 0.25, 5.0))
	_library["land"] = _mix([
		_noise(0.2, 0.4, 9.0, 0.6),
		_sweep(0.22, 190.0, 60.0, 0.35, 5.0),
	])
	_library["health"] = _mix([
		_tone(0.28, 880.0, 0.22, 5.0),
		_tone(0.34, 1320.0, 0.16, 4.0),
	])
	_library["jump_pad"] = _sound(_sweep(0.4, 200.0, 1100.0, 0.4, 2.2))
	_library["ui_click"] = _sound(_tone(0.07, 1400.0, 0.25, 22.0))
